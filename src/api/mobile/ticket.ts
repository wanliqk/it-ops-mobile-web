...
import type { PageResult, Ticket, TicketCreateForm, TicketFormOptions, TicketListQuery } from '@/types'
...
/** 创建报修工单 */
export function createTicket(data: TicketCreateForm): Promise<Ticket> {
  return request({ url: '/api/mobile/tickets', method: 'post', data })
}

/** 我的报修列表（分页 + 状态筛选 + 关键词搜索） */
export function getTickets(query: TicketListQuery): Promise<PageResult<Ticket>> {
...
/** 取消报修 */
export function cancelTicket(id: string | number, reason?: string): Promise<void> {
